...
import type { DashboardData, GoldPrice, HistoricalPrice, IndicatorSeries, Timeframe } from "@/types";

// Lazy-load heavy components — they are not needed on first paint
const GoldChart = dynamic(
  () => import("./GoldChart").then((m) => ({ default: m.GoldChart })),
  { ssr: false, loading: () => <Skeleton className="h-[560px] rounded-2xl bg-white/5" /> }
);
const MarketCommentary = dynamic(
  () => import("./dashboard/MarketCommentary").then((m) => ({ default: m.MarketCommentary })),
  { ssr: false, loading: () => <Skeleton className="h-32 rounded-2xl bg-white/5" /> }
);
const NewsSentimentCard = dynamic(
  () => import("./dashboard/NewsSentimentCard").then((m) => ({ default: m.NewsSentimentCard })),
  { ssr: false, loading: () => <Skeleton className="h-32 rounded-2xl bg-white/5" /> }
);
const CopilotChat = dynamic(
  () => import("./chat/CopilotChat").then((m) => ({ default: m.CopilotChat })),
  { ssr: false }
);
...
// Phase-1 state: price + chart (fast)
type PriceState = {
  price: GoldPrice;
...
export function Dashboard() {
  // Phase 1 — price arrives fast (~200ms from Yahoo Finance)
  const [priceState, setPriceState] = useState<PriceState | null>(null);
  // Phase 2 — full data incl. AI signal (may take 1-3s if cache miss)
  ...
  const fetchData = useCallback(
    async (tf: Timeframe = timeframe, silent = false) => {
      if (!silent) setSignalLoading(true);
      if (!silent) setRefreshing(true);
      try {
        // Single call — but price/chart renders as soon as JSON arrives
        const res = await fetch(`/api/analyze?timeframe=${tf}`);
        if (!res.ok) throw new Error("fetch failed");
        const json: DashboardData = await res.json();
        const full = { ...json, timeframe: tf };

        setPriceState({
          price: json.price,
          history: json.history,
          indicatorSeries: json.indicatorSeries,
        });
        setData(full);
  ...
  // Nothing loaded yet
  if (!priceState && signalLoading) return <DashboardSkeleton />;
  ...
  const price = data?.price ?? priceState!.price;
  const history = data?.history ?? priceState!.history;
  const indicators = data?.indicators;
  const indicatorSeries = data?.indicatorSeries ?? priceState!.indicatorSeries;
  const signal = data?.signal;

  const signalColor =
    signal?.signal === "BUY" ? "text-emerald-400"
    : signal?.signal === "SELL" ? "text-red-400"
    : "text-yellow-400";

  const signalBg =
    signal?.signal === "BUY" ? "bg-emerald-500/10 border-emerald-500/30"
    : signal?.signal === "SELL" ? "bg-red-500/10 border-red-500/30"
    : "bg-yellow-500/10 border-yellow-500/30";

  const TrendIcon = price.trend === "up" ? TrendingUp : price.trend === "down" ? TrendingDown : Minus;
  const trendColor = price.trend === "up" ? "text-emerald-400" : price.trend === "down" ? "text-red-400" : "text-zinc-400";
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-4 md:p-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-bold text-white md:text-2xl">{t("dashboard.title")}</h1>
        ...
        <TabsList className="bg-white/5">
          {TIMEFRAMES.map((tf) => (
            <TabsTrigger
              key={tf}
              value={tf}
              className="text-xs data-[state=active]:bg-yellow-500 data-[state=active]:text-zinc-950"
            >
              {t(`timeframes.${tf}`)}
            </TabsTrigger>
          ))}
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {/* Price card — renders immediately */}
          <motion.div
          ...
          >
            ...
            <div className="mt-3 flex flex-wrap gap-4 text-xs text-zinc-500">
              ...
              <span className={`font-medium ${trendColor}`}>{t(`price.trend.${price.trend}`)}</span>
            </div>
          </motion.div>

          {/* Signal card — skeleton while AI loads */}
          <motion.div
            className={`rounded-2xl border p-6 backdrop-blur-xl ${signal ? signalBg : "border-white/10 bg-white/5"}`}
            whileHover={{ scale: 1.005 }}
          >
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {signal.takeProfit && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-emerald-400">${signal.takeProfit.toFixed(2)}</span>
                    </div>
                  )}
                </div>
              </>
      ...
      {signal && (
        ...
        >
          ...
          <details className="mt-3 group">
            <summary className="cursor-pointer list-none flex items-center gap-1 text-xs text-yellow-500 hover:text-yellow-400">
      ...
      {/* Chart — lazy loaded, renders immediately once bundle arrives */}
      <motion.div
        variants={fadeUp}
        initial="hidden"
        animate="visible"
      ...
      >
        <GoldChart data={history} indicatorSeries={indicatorSeries} />
      </motion.div>

      {/* Indicators card — only when data available */}
      {indicators && (
        <motion.div
          variants={fadeUp}
          initial="hidden"
      ...
      {/* Floating AI chat — lazy loaded, only fetches bundle on first open */}
...
function DashboardSkeleton() {
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-4 md:p-6">
      ...
      <Skeleton className="h-[560px] rounded-2xl bg-white/5" />
